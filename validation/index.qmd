...
adjCNRM_CDFt = readRDS("output_biascorrection/biascorrectedCNRM-CDFt.rds")
lapply(adjCNRM_CDFt, function(df) head(df, 5))
```

# Validation of the MBCn bias correction using calibrated GR4J hydrological model over the historical period
...
daily_cycle_plot$Obs_smooth = rollmean(daily_cycle_plot$Obs, k = 20, fill = "extend")
daily_cycle_plot$gr4jObs_smooth = rollmean(daily_cycle_plot$gr4jObs, k = 20, fill = "extend")
daily_cycle_plot$gr4jRaw_smooth = rollmean(daily_cycle_plot$gr4jRaw, k = 20, fill = "extend")
daily_cycle_plot$gr4jAdj_smooth = rollmean(daily_cycle_plot$gr4jAdj, k = 20, fill = "extend")
...
par(mfrow = c(1, 3), mar = c(4.5, 4.5, 3, 1), family = "serif", pty = "s")
...
for(r in rows) {
  for(m in metrics) {
    ...
    qqPlot(x = obs_vals, 
           ...
           ylim = val_range,
           main = paste(m$title),
           xlab = "GR4J with bias-corrected RCM data",
...
# Validation of the CDFt bias correction using calibrated GR4J hydrological model over the historical period
