...
# Validation of the bias correction using calibrated GR4J hydrological model over the historical period

In this section, we will run hydrological experiments or simulations using calibrated GR4J model driven  by observed hydroclimatic data and bias-corrected RCMs outputs. We first load the optimized GR4J parameters derived in the [GR4J model calibration section](../modelcalib/index.qmd#sec-gr4jcalib).
...
gr4jsim_histCLM_raw = run_gr4j_simulation(
  na.omit(adjCLM[["histCLM_adj"]]), "PR", "PET", Qobs_data, gr4j_param
)
head(gr4jsim_histCLM_raw)

gr4jsim_histCLM_adj = run_gr4j_simulation(
  adjCLM[["histCLM_adj"]], "PR_adj", "PET_adj", Qobs_data, gr4j_param
)
head(gr4jsim_histCLM_adj)
```
...
RawhistGR4JclimAdjMetrics = compute_hydro_indices(
  gr4jsim_histCLM_raw, "GR4J with raw RCM data", "Qsim"
)
...
AdjhistGR4JclimAdjMetrics = compute_hydro_indices(
  gr4jsim_histCLM_adj, "GR4J with bias-corrected data", "Qsim"
)
...
#| fig-cap: "Mean daily discharge simulated by GR4J using observed data, raw RCM outputs, and bias-corrected RCM data (5-day rolling mean)."

# Combine observed, raw, and adjusted mean daily cycles into a single data frame
daily_cycle_plot = histGR4JObsmeanDaily %>%
  select(HydroDOY, Obs = MeanDailyQ) %>%
  left_join(RawhistGR4JclimAdjmeanDaily %>% select(HydroDOY, Raw = MeanDailyQ), by = "HydroDOY") %>%
  left_join(AdjhistGR4JclimAdjmeanDaily %>% select(HydroDOY, Adj = MeanDailyQ), by = "HydroDOY")

# 5-day moving average to reduce noise and highlight the seasonal signal
daily_cycle_plot$Obs_smooth = rollmean(daily_cycle_plot$Obs, k = 5, fill = "extend")
daily_cycle_plot$Raw_smooth = rollmean(daily_cycle_plot$Raw, k = 5, fill = "extend")
daily_cycle_plot$Adj_smooth = rollmean(daily_cycle_plot$Adj, k = 5, fill = "extend")

par(mfrow = c(1, 1), mar = c(5, 5, 2, 2), family = "serif")

y_range = range(c(daily_cycle_plot$Obs_smooth, 
                   daily_cycle_plot$Raw_smooth, 
                   daily_cycle_plot$Adj_smooth), na.rm = TRUE)

plot(daily_cycle_plot$HydroDOY, daily_cycle_plot$Obs_smooth, 
     type = "l", lwd = 1.1, col = "black",
     ylim = y_range,
     xlab = "Hydrological day of year", 
...
lines(daily_cycle_plot$HydroDOY, daily_cycle_plot$Raw_smooth, 
      col = "red", lwd = 1.1, lty = 2)

lines(daily_cycle_plot$HydroDOY, daily_cycle_plot$Adj_smooth, 
      col = "blue", lwd = 1.1, lty = 3)

grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")

legend("topright", 
       legend = c("GR4J with observed data", 
                  "GR4J with raw RCM", 
...
par(mfrow = c(2, 3), mar = c(4.5, 4.5, 3, 1), family = "serif", pty = "s")

for(r in rows) {
  for(m in metrics) {
    
    # Extract the vectors (removing NAs is vital for Q-Q math)
    obs_vals = na.omit(plot_data[[m$obs]])
    sim_col  = paste0(r, m$var)
    sim_vals = na.omit(plot_data[[sim_col]])
    
    # Calculate the range based on both distributions to keep the plot square
    val_range = range(c(obs_vals, sim_vals), na.rm = TRUE)
    
    # Styling logic
    pt_color = ifelse(r == "Raw", "#ffb04e", "#5aa4ff")
    row_lab  = ifelse(r == "Raw", "Raw RCM", "Bias-Corrected RCM")
    
    # Use EventStat's qqPlot
    # This automatically handles the sorting and quantile calculation
    qqPlot(x = obs_vals, 
           y = sim_vals,
           plot.type = "Q-Q",
           qq.line.type = "0-1", # Adds the 1:1 reference line
           add.line = TRUE,
           xlim = val_range, 
           ylim = val_range,
           points.col = pt_color,
...
# Validation of the bias correction using calibrated IHACRES hydrological model
...
::: {.callout-tip title="Tip: Run the calibrated IHACRES model using historical RCM data"}
