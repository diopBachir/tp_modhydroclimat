...
The database includes climate projections from three different RCMs, organized into specific folders:

* **CLM:** Community Land Model / Community Earth System Model (USA).
...
**Example:** **`clm_dataTMAX_rcp85.csv`** represents the Maximum Temperature projections for the Lez basin using the CLM model under the pessimistic RCP8.5 scenario.
:::
...
clm_data = prepare_climate_data(
    "clm", climdatapath, hist_start, hist_end, fut_start, fut_end
)
...
ipsl_data = prepare_climate_data(
    "ipsl", climdatapath, hist_start, hist_end, fut_start, fut_end
)
```
...
models_list = list(
    climdataCLM = clm_data,
    climdataCNRM = cnrm_data,
    climdataIPSL = ipsl_data
)
