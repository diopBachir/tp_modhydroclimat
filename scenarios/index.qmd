...
As in the previous sections, we source the [**`run_gr4j_simulation(...)`**](https://github.com/diopBachir/tp_modhydroclimat/blob/main/helper_functions/prepare_climate_data.R) helper function that prepares all required input objects for the GR4J model using already calibrated parameters.
...
Let's plot the mean daily and monthly  discharge for the historical, RCP4.5 and RCP8.5 simulations. We source, in the chunk below, the [**`compute_mean_daily_cycle(...)`**](https://github.com/diopBachir/tp_modhydroclimat/blob/main/helper_functions/prepare_climate_data.R) helper function.
