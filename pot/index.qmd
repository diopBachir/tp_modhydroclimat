...
We will be working with the same data used in the [**block maxima section**](../blockmaxima/index.qmd#sec-bmdata), from the **`La Drôme à Luc-en-Diois`** station in France. 

::: {.callout-note icon="false"}
## Download the Data used in this Section

<a href="https://github.com/diopBachir/tp_eva/blob/master/pot/data/discharge.csv" download class="download-btn">📥 Download CSV</a>
:::
...
While the optimal threshold is usually determined through diagnostic tools, a reasonable starting point is needed to begin the analysis. Here, we use the **90th percentile** of the discharge data — as an initial threshold. This choice balances the need for sufficient exceedances to ensure statistical stability while focusing on the upper tail of the distribution where extreme behavior occurs. The snippet below extract all flows above the 90th percentile, and plots the histogram of these values. 

```{r}
#| echo: false
#| fig-height: 4
#| label: fig-allexcedences
#| fig.cap: "Streamflow time serie with all exceedances (red dots) above the initial threshold (the 90th percentile represented by the horizontal blue line)"

# extract all flow values above the threshold and sort by date
q90 = quantile(Qdata$Q, probs = 0.90, na.rm=TRUE)
q90_exceedances = Qdata |>
                  filter(Q > q90) |>
                  arrange(Date)

plot(Qdata$Date, Qdata$Q, type = "l", col = "grey", main = "", xlab="Time", ylab="Streamflow (Q)")
points(q90_exceedances$Date, q90_exceedances$Q, col = "red", pch = 19, cex=.5)
abline(h = q90, col = "blue", lty=1, lwd=2) 
```

The **correlogram** (@fig-acfplot_pot) shows that the extracted peaks are autocorrelated for lag 1, 2 and 3. This reflects dependence between consecutive peaks, and even one or two time step apart, values still influence each other. 
...
bacf = acf(q90_exceedances$Q, plot = FALSE)
bacfdf = with(bacf, data.frame(lag, acf))

# confidence interval
n = length(q90_exceedances$Q)
...
![Inter-flood duration criteria (adapted from @lang1999). $θ$ represents a minimum number of days between two consecutive peaks. $(Xs)_1$ and $(Xs)_2$ are respectively the first and second peaks, and $X_{min}$ is the minimum flow value between the two peaks.](images/declust_modern.png){#fig-decluster}
...
$$Ts = 5days + log(A)$$
where $A$ is the catchment area. Let $θ$ represents the minimum number of days between two consecutive peaks: 

    - If $θ < Ts$ → **`REJECT`** the second peak.
...
2. **`Magnitude differential rule:`** flow should drop below a certain fraction of the smaller peak between events. @cunnane1979 propose the following condition, where $X_{min}$ is the minimum flow value between the two peaks, and $(Xs)_1$ is the first peak:
    ...
    - If $X_{min} > (2/3)(Xs)_1$ → **`REJECT`** the second peak $(Xs)_2$. 
...
```{r}
thres = q90   # threshold value
area = 194    # catchment area
```


We first define a function which takes as arguments the **extremes flows above the 90th percentile extracted below**, the **threshold** value, and the **drainage basin area**, and performs the **temporal declustering technique**:
...
# time separation
peaks_time_declust = time_sep_declust(data=q90_exceedances, threshold=thres, Area=area)
```
...
#| fig.cap: "Streamflow time serie with all exceedances (red dots) above the threshold (90th percentile - horizontal blue line), along with the declustered peaks (green crosses) using the time-separation rule."
...
#| fig.cap: "Streamflow time serie with all exceedances (red dots) above the threshold (90th percentile - horizontal blue line), along with the declustered peaks (green crosses) using the time-separation rule, and the declustered peaks (blue dots) using the magnitude difference rule."

# plot original vs declustered peaks
plot(Qdata$Date, Qdata$Q, type = "l", col = "grey", main = "", xlab="Time", ylab="Streamflow (Q)")
points(q90_exceedances$Date, q90_exceedances$Q, col = "red", pch = 19, cex=.5)
points(peaks_time_declust$Date, peaks_time_declust$Q, col = "green", pch = 3, cex=.7)
points(peaks_final$Date, peaks_final$Q, col = "blue", pch = 19, cex=.5)
abline(h = thres, col = "blue", lty=1, lwd=2) 
```
...
To further assess the independence of the declustered peaks, we apply the Wald-Wolfowitz independence test. From the results displayed below, we conclude that **the declustering worked well**, as we now fail to reject the null hypothesis of independance, with **p.value equal to 0.72**.
...
# Refining (tuning) the threshold value {#sec-thresholdident}
...
par(cex = 1.7)
suppressWarnings(POT::tcplot(peaks_final$Q, u.range = c(5, 30)))
```
...
x = na.omit(peaks_final$Q)
POT::mrlplot(x, col=c("green", "black", "green"), lty=c(2, 1, 2), nt=200, main="")
abline(v = 10, col = "red", lty = 2, lwd = 2) 
```


Finally, we can extract the extreme values above the threshold $q = 10$ and use the in the next sections.

```{r}
#| echo: true
#| label: fig-finalpeak
#| fig-height: 5
#| fig-cap: "Streamflow time serie with the final peaks extracted (blue dots) above the threshold $q = 10$, represented by red horizontal line."

# extract final peaks
thres = 10
peaks_final = filter(peaks_final, Q > thres)

# plot the final peaks
plot(Qdata$Date, Qdata$Q, type = "l", col = "grey", main = "", xlab="Time", ylab="Streamflow (Q)")
points(peaks_final$Date, peaks_final$Q, col = "blue", cex=.5)
...
::: {.callout-tip}
## Interpretation
**`p.value = 0.3846:`** there is not enough statistical evidence to conclude that a significant trend exists in the threshold exceedances serie.

**`tau = -5.233612e-02:`** indicates a weak or no trend.

**`z = -0.86943:`** suggests a potential decreasing trend, but the trend is not significant.
:::
:::
...
The threshold exceedance show a slight decreasing trend with a **slope of -0.0056 mm/year**, which is equivalent to a overall decrease of **-4.71% relative to its long-term average**. Yet, this negative trend is not significative given the **p-value of 0.3846**.
:::
:::
...
As we have already seen in the previous section, the **GP** distribution is the most appropriate distribution to model threshold exceedances. As explained by @gilleland2016extremes in their paper, an important additional consideration when using the POT approach is the **`rate of exceedance` — that is, how often values exceed the chosen threshold. While this rate does not affect the fitting of the GP model itself, it is crucial for calculating return levels, which require scaling the model to real-world time units.`**
...
thres = 10
x = peaks_final$Q
sgp_fit = fevd(x, type="GP", method="GMLE", threshold=10, time.units = "2.56/year")
...
**`sigma0 = σ0 = 5.7161:`** is the **intercept** of the time-varying GP scale parameter. It represents the base scale of the distribution (often the start of the time series). 

**`sigma1 = σ1 = -0.0128:`** is the **slope** of the scale parameter with respect to time. This negative value suggests that extreme values are becoming less variable (narrower tail) over time.

**`shape = 0.0417:`** controls the **heaviness** of the tail of the GP distribution. The positive value (ξ > 0) suggests a heavy (Fréchet) tail (i.e., the probability of very large exceedances decays slowly).
:::
:::
...
The null hypothesis of the **deviance test** is that the **simpler model is sufficient — i.e., no trend** (e.g., the GP scale parameter is constant over time). See the [Deviance test Section](../blockmaxima/index.qmd##sec-deviancetest) for the mathematical details. 
...
* **`D-statistic (1.0848)`** much smaller than the **`critical value (3.8415)`**.
* **`p-value (0.2976)`** much larger than the **`significance level (0.05)`**.
:::
:::
...
* The **Likelihood Ratio Test** showed a high p-value (0.2976), and both the **Mann-Kendall test** and the **Sen's slope** similarly returned a non-significant result, with a p-value of 0.3846.
...
In non-stationary context, since the **scale parameter is time-dependent**, **return levels** have to be calculated for each time step. We refer to this as **`“effective” return levels`**The same **`return.level(...)`** from the **`{extRemes}`** package is used to compute **`"effective" return levels`**.
